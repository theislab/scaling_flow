...
class TestCellFlow:
    ...
    def test_cellflow_solver_with_torch_dataloader(
        self,
        adata_perturbation,
        tmp_path,
    ):
        ...
        cf.train_data.write_zarr(Path(tmp_path) / "train_data1.zarr")
        cf.train_data.write_zarr(Path(tmp_path) / "train_data2.zarr")
        cf.train_data.write_zarr(Path(tmp_path) / "train_data3.zarr")
        ...
        assert cond_embed_var.shape[0] == conds.shape[0]
        assert cond_embed_var.shape[1] == condition_embedding_dim
        # TODO: add mode to make the training independent of anndata or at least make it
        # read only