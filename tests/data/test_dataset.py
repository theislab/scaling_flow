...
class TestPerturbationData:
    ...
    def test_load_from_adata_no_combinations(
        ...
        if obs_perturbation_covariates == ["dosage"] and uns_perturbation_covariates == []:
            assert pdata.n_perturbations == len(adata_perturbation.obs["dosage"].cat.categories)
        if obs_perturbation_covariates == [] and uns_perturbation_covariates == {"drug": ("drug1",)}:
            assert pdata.n_perturbations == (len(adata_perturbation.obs["drug1"].cat.categories) - 1) * pdata.n_controls
        assert isinstance(pdata.cell_data, jax.Array)
        assert isinstance(pdata.split_covariates_mask, jax.Array)
        assert isinstance(pdata.split_covariates_to_idx, dict)
        assert isinstance(pdata.perturbation_covariates_mask, jax.Array)
        assert isinstance(pdata.perturbation_covariates_to_idx, dict)
        assert isinstance(pdata.control_to_perturbation, dict)

    @pytest.mark.parametrize("split_covariates", [[], ["cell_type"]])
    @pytest.mark.parametrize("control_data", [("drug1", "Vehicle")])
    @pytest.mark.parametrize("obs_perturbation_covariates", [[], [["dosage"]]])
    @pytest.mark.parametrize("uns_perturbation_covariates", [{"drug": ("drug1", "drug2")}])
